...
const CourseSchema = new mongoose.Schema({
  ...
  statut: { type: String, enum: ['en_attente','acceptée','refusée','annulée','terminée'], default: 'en_attente' },
...
app.post('/api/inscription', async (req, res) => {
  ...
  if (!['client','chauffeur'].includes(role)) return res.status(400).json({ error: 'Rôle invalide' });
  if (role==='chauffeur'&&!plaque) return res.status(400).json({ error: 'Plaque requise' });
  try {
    if (await User.findOne({ whatsapp })) return res.status(409).json({ error: 'Numéro WhatsApp déjà utilisé' });
...
app.post('/api/connexion', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: 'Numéro WhatsApp introuvable' });
...
app.post('/api/course/demander', authMiddleware, async (req, res) => {
  ...
  if (await hasCourseActive(clientId)) return res.status(409).json({ error: 'Vous avez déjà une course en cours' });
  ...
  try {
    ...
    drivers[chauffeurId].statut = 'occupé';
...
app.post('/api/course/annuler', authMiddleware, async (req, res) => {
  ...
  try {
    const course = await Course.findOne({ _id: courseId, $or: [{ clientId: req.user.id }, { chauffeurId: req.user.id }], statut: { $in: ['en_attente','acceptée'] } });
    if (!course) return res.status(404).json({ error: 'Course introuvable' });
    if (req.user.role==='client' && course.statut==='acceptée') {
      const elapsed = (Date.now() - new Date(course.acceptedAt).getTime()) / 60000;
      if (elapsed > 2) return res.status(409).json({ error: 'Délai d\'annulation dépassé (2 min). Contactez le chauffeur sur WhatsApp.' });
    }
    course.statut = 'annulée'; await course.save();
    ...
    if (req.user.role==='client') {
      ...
      if (s) io.to(s).emit('course_annulée', { courseId, par: 'client' });
    } else {
      const s = clients[course.clientId.toString()]?.socketId;
      if (s) io.to(s).emit('course_annulée', { courseId, par: 'chauffeur' });
    }
    io.emit('drivers_updated', await getDriversList());
    res.json({ ok: true });
  } catch(err) { res.status(500).json({ error: 'Erreur serveur' }); }
...
app.post('/api/course/terminer', authMiddleware, async (req, res) => {
  ...
  const { courseId } = req.body;
  try {
    const course = await Course.findOne({ _id: courseId, chauffeurId: req.user.id, statut: 'acceptée' });
    if (!course) return res.status(404).json({ error: 'Course introuvable' });
    course.statut = 'terminée'; course.finishedAt = new Date(); await course.save();
    const chauffeurId = req.user.id;
    if (drivers[chauffeurId]) drivers[chauffeurId].statut = 'libre';
    const s = clients[course.clientId.toString()]?.socketId;
    if (s) io.to(s).emit('course_terminée', { courseId });
    io.emit('drivers_updated', await getDriversList());
    res.json({ ok: true });
  } catch(err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
